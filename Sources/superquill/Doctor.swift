...
enum DoctorReport {
    ...
    static func checkMicrophone() -> Check {
        ...
        case .denied, .restricted:
            return Check(
                name: "microphone",
                status: .fail("denied"),
                remediation: "System Settings → Privacy & Security → Microphone → enable for superquill (or your terminal)"
            )
        @unknown default:
    ...
    static func checkTranscription() -> Check {
        ...
        // Check the cache for whichever model the config actually selects.
        let version: AsrModelVersion = Config.transcriptionEngine() == "parakeet-v2" ? .v2 : .v3
